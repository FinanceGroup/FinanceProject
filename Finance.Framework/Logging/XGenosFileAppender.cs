...
namespace Finance.Framework.Logging {
    public class XGenosFileAppender : RollingFileAppender {
        /// <summary>
            ...
            lock (this) {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++) {
                    try {
                        ...
                        LogLog.Error(typeof(XGenosFileAppender), string.Format("XGenosFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
