...
namespace Finance.Framework.Data
{
    public class SessionLocator : ISessionLocator, ITransactionManager, IDisposable {
        ...
        public void RequireNew(IsolationLevel level) {
            ...
            //if (IsolationLevel.ReadUncommitted == level
            //    && _sessionFactoryHolder.GetConfiguration().Properties["dialect"].Contains("SqlCe")) {
            //    Logger.Information("The IsolationLevel ReadUncommitted is not supported by SqlCe, ignored!", level);
                ...
                try {
                    if (!_cancelled) {
                        ...
                        try {
                            ...
                            Logger.Error(ec, "Error while committing the transaction.");
                            _transaction.Rollback();
                        }
                    }
                    else {
        ...
        private void EnsureSession() {
            ...
            _session = sessionFactory.OpenSession(new XGenosSessionInterceptor(_interceptors.ToArray(), Logger));
        }

        class XGenosSessionInterceptor : IInterceptor {
            private readonly ISessionInterceptor[] _interceptors;
            private readonly ILogger _logger;

            public XGenosSessionInterceptor(ISessionInterceptor[] interceptors, ILogger logger) {
                _interceptors = interceptors;
                _logger = logger;
            }
            ...
            //    return ex is XGenosSecurityException || !ex.IsFatal();
            //}
        }
    }
}